...
function calcWeekProgressFromStore(store, weekStartKey) {
  ...
  if (!start) return { bjjSessions: 0, pushups: 0, rows: 0, kbSwings: 0 }
  ...
  return inWeek.reduce((acc, key) => {
    ...
    return acc
  }, { bjjSessions: 0, pushups: 0, rows: 0, kbSwings: 0 })
}
...
function weeklyHit(progress, targets) {
  return {
    ...
    all: progress.bjjSessions >= targets.bjjSessions && progress.pushups >= targets.pushups && progress.rows >= targets.rows && progress.kbSwings >= targets.kbSwings
  }
}
