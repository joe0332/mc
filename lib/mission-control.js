...
export function getDashboardData() {
  if (isSnapshotMode()) {
    return readSnapshot('dashboard', { jobs: [], recent: [], tasks: { pending: [], completed: [] }, offloadInsights: { timeSavedThisWeek: { minutes: 0, hours: 0, breakdown: [] }, topOffloadNext: { title: '—', reason: '', source: '' } }, lastRefreshedAt: null })
  }
  const db = ensureDb()
...
export function getKanbanBoard(includeArchived = true) {
  if (isSnapshotMode()) {
    const snap = readSnapshot('kanban', { cards: [] })
    if (includeArchived) return snap
    return { cards: (snap.cards || []).filter((c) => !c.isArchived) }
  }
  const db = ensureDb()
...
export function getJobDetails(jobId) {
  ...
  try {
    const raw = execSync(`openclaw cron runs --id ${jobId} --limit 5 --json`, {
      cwd: ROOT,
      stdio: ['ignore', 'pipe', 'pipe'],
      encoding: 'utf8'
    })
    const parsed = JSON.parse(raw)
    runs = parsed.entries || []
    lastErrorText = runs.find((r) => r.status && r.status !== 'ok')?.summary || ''
  } catch {
    try {
      const rawText = execSync(`openclaw cron runs --id ${jobId} --limit 5`, {
        cwd: ROOT,
        stdio: ['ignore', 'pipe', 'pipe'],
        encoding: 'utf8'
      })
      lastErrorText = rawText.trim()
    } catch (err) {
      const stderrText = err?.stderr ? String(err.stderr).trim() : ''
      const stdoutText = err?.stdout ? String(err.stdout).trim() : ''
      lastErrorText = stderrText || stdoutText || err?.message || ''
    }
  }
