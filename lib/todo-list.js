...
const DATA_PATH = path.join(WORKSPACE_ROOT, 'memory', 'todo-list.json')
...
export function getTodoListData() {
  try {
    const liveData = readJsonFileSafe(DATA_PATH, { ...DEFAULT_DATA })

    const normalizedLive = {
      rows: Array.isArray(liveData?.rows) ? liveData.rows : [],
      updatedAtMs: Number(liveData?.updatedAtMs || 0) || null
    }

    if (isSnapshotMode()) {
      const snap = readSnapshot('todo-list', { ...DEFAULT_DATA })
      const snapRows = Array.isArray(snap?.rows) ? snap.rows : []
      const snapUpdatedAtMs = Number(snap?.updatedAtMs || 0) || null

      // Important: an empty/stale snapshot must not mask the real to-do list.
      if (snapRows.length > 0 || snapUpdatedAtMs || normalizedLive.rows.length === 0) {
        return {
          rows: snapRows,
...
export function saveTodoListData(payload = {}) {
  ...
  const cleaned = rows.map((r, i) => ({
    ...
    dueDate: normalizeDueDate(r?.dueDate),
