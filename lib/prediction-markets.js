...
const CSV_PATH = path.join(ROOT, 'weather-paper-trade-journal.csv')
...
export async function getPredictionMarketsOverview() {
  if (isSnapshotMode()) {
    return readSnapshot('prediction-markets', { rows: [], trades: [], metrics: { totalRows: 0, totalCommits: 0, resolvedCommits: 0, wins: 0, losses: 0, winRate: null, avgCommitEdge: null, avgLatestEdge: null }, updatedAtMs: null })
  }
  ...
  const totalResolvedWager = Math.round(resolved.reduce((sum, t) => {
    const wager = Number(t.estimatedWager)
    if (!Number.isFinite(wager)) return sum
    return sum + wager
  }, 0) * 100) / 100
  const realizedRoiPct = totalResolvedWager > 0 ? safePct((realizedNetPnl / totalResolvedWager) * 100) : null
