...
export default function PredictionMarketsPage() {
  ...
  async function load() {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/prediction-markets', { cache: 'no-store' })
  ...
  async function runSettlementCheck() {
    ...
    setError('')
    try {
      const res = await fetch('/api/prediction-markets', { method: 'POST' })
      const json = await res.json()
  ...
  return (
    <main style={{ minHeight: '100vh', padding: 24, background: '#0b1020', color: '#e8ecf3', fontFamily: 'Inter, Segoe UI, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 30 }}>Prediction Markets Overview</h1>
          <p style={{ margin: '6px 0 0', opacity: 0.8 }}>Paper-trade commits, edge evolution, and win/loss tracking.</p>
        ...
        <>
          <section style={{ background: '#121a33', borderRadius: 12, padding: 14, marginTop: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, minmax(0, 1fr))', gap: 8 }}>
              ...
              <div style={{ border: '1px solid #334155', borderRadius: 8, padding: 10, background: '#0f172a' }}>
                <div style={{ opacity: 0.72, fontSize: 12 }}>Closed ROI</div>
                <div style={{ fontSize: 24, fontWeight: 700, color: (data.metrics?.realizedRoiPct ?? 0) > 0 ? '#86efac' : (data.metrics?.realizedRoiPct ?? 0) < 0 ? '#fca5a5' : '#e8ecf3' }}>{pct(data.metrics?.realizedRoiPct)}</div>
                <div style={{ opacity: 0.65, fontSize: 11, marginTop: 4 }}>On {usd(data.metrics?.totalResolvedWager ?? 0)} resolved stake</div>
              </div>
